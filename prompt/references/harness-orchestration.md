...
Do **not** insert plan mode, "PAUSE for approval," "your call," billing asides, or operator notes inside the mission — those break the autonomous run. But know what the model can and cannot do: `/code-review`, `/security-review`, and subagent fan-out are model-invocable and belong inline; **`/goal`, permission mode, and the `ultracode` keyword are human-only switches** — `/goal` is a built-in the model cannot run, mode changes only via Shift+Tab or a launch flag, and `ultracode` fires only from human-typed input. So the launch is the prompt plus the human-only switches — and *where the run starts* decides their form. Inside an interactive session (desktop app or running CLI — the default, since that is where the skill is invoked from), auto mode is already on and CLI flags cannot be run, so the launch is two chat messages: `/goal <condition>` as its own message, then the mission starting with `ultracode`; optional one-time `/effort max` and a `/permissions` deny for the irreversible edge. Only for a new shell session or headless/CI is it the single launch line `claude --permission-mode auto --effort max --fallback-model sonnet,haiku --max-budget-usd <n> --allowedTools "<tools>" --disallowedTools "<irreversible edge, e.g. Bash(git push *)>" "/goal <condition>"` then the mission paste. Either way it is not a multi-step ritual: resume behavior, re-reading the plan, survival, and gates live inside the prompt; the only other human action is reopening the session (`claude -c`) after a crash. A `/goal` buried mid-prompt is inert text — slash commands fire only at the start of a message — and a run with no real goal-gate stops at its first turn boundary and asks "keep going?".
