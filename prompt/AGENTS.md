...
- `references/` — `evidence-standard.md`, `task-adapters.md`, `evaluation-protocol.md`, `harness-orchestration.md` (Claude Code), `codex-adapter.md` (Codex/GPT), `openclaw-adapter.md` (OpenClaw), `long-horizon-autonomy.md` (surviving compaction over long runs), `context-graph-loop.md` (per-task execution loop + loop breakers), `rpi-adapter.md` (RPI delivery: gated spec→research→plan→implement→review on current-state proof), `claude-code-commands.md`, `session-cases.csv`.
