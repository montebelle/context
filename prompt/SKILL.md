...
- `Launch:` REQUIRED for every orchestration rewrite. The ordered messages the human must send, because the model cannot flip these itself: (1) set auto mode — Shift+Tab, or start the session with `--permission-mode auto`; (2) send `/goal <whole-mission condition; save progress to a file; don't stop on token budget; stop after N rounds>` as its OWN message; (3) send the mission prompt, beginning with the word `ultracode`. A `/goal` or mode change buried inside pasted prose is inert text — slash commands fire only at the start of a message, and `/goal` is a human-only built-in. Model-invocable skills (`/code-review`, `/security-review`, subagent fan-out) stay woven inside the prompt.
- `Harness controls:` genuine out-of-prompt controls only (access, credentials, isolation, rollback, security enforcement). Human-only switches go under `Launch`; model-invocable skills stay inline.
...
- For long-horizon or unattended orchestration prompts, include the long-run survival block (`references/long-horizon-autonomy.md`): durable external progress, re-grounding after compaction, self-verification, self-healing with bounded retry, resumable checkpoints. Never assume in-context memory survives compaction.
