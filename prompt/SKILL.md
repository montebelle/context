...
- `Launch:` REQUIRED for every orchestration rewrite. The ordered messages the human must send, because the model cannot flip these itself: (0) once, `/fewer-permission-prompts` (or `/permissions` allow rules) so the run never stalls on a permission; (1) start the session with `claude --permission-mode auto --fallback-model sonnet,haiku --max-turns <N>` (add `--max-budget-usd` if cost-bounded) — or Shift+Tab to auto mode if already in session; (2) send `/goal <whole-mission condition; save progress to a file; don't stop on token budget; don't delete or weaken tests; stop after N rounds>` as its OWN message; (3) send the mission prompt, beginning with the word `ultracode`; (4) if the session dies: `claude -c` (or `-r <session>`), then `/recap`, then re-ground from the progress file — the `/goal` persists with the session, so resume, don't relaunch. A `/goal` or mode change buried inside pasted prose is inert text — slash commands fire only at the start of a message, and `/goal` is a human-only built-in. Model-invocable skills (`/code-review`, `/security-review`, `/verify`, `/simplify`, subagent fan-out) stay woven inside the prompt.
