...
- `Launch:` REQUIRED for every orchestration rewrite, and it is ONE launch line, not a ritual. Every human-only switch rides as a CLI flag; everything the agent can do (resume behavior, re-reading the plan, survival, gates) lives inside the prompt, never here. Emit exactly this shape:
